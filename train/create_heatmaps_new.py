...
from models.att_model import CLAM_SB_Reg_NN_Pool, CLAM_MB_Reg
...
parser.add_argument('--config_file', type=str, default="config_21.yaml")
args = parser.parse_args()

def infer_single_slide(model, features, label, label_dict, k=1):
	features = features.to(device)
	with torch.no_grad():
		if isinstance(model, (CLAM_SB_Reg_NN_Pool, CLAM_MB_Reg)):
			A = model(features,attention_only = True)
			logits = model(features)

			if isinstance(model, (CLAM_MB_Reg,)) and A.size(0) > 1:
				A = A[label_dict[label]]
				logits = logits[label_dict[label]]
			
			A = A.view(-1, 1).cpu().numpy()
			logits = logits.view(-1, 1).cpu().numpy()
...
def initiate_model(args, ckpt_path):
	print('Init Model')
	model_dict = {"dropout": args.drop_out, 'n_classes': args.n_classes, 'size_arg': args.model_size}
...
def load_model(model_name, weight_path):
	check_point = torch.load(weight_path)#,map_location=torch.device('cpu')
	...
	if model_name == "mae":
		...
		model = None  # models_mae.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
		new_sd = OrderedDict()
		...
		return model
	elif model_name.startswith("mocov2") or model_name.startswith("ADCO_tcga"):
...
if __name__ == '__main__':
	...
	#/data/data_zy/experiments/adco/breast_1/
	feature_extractor = load_model(model_name="ADCO",weight_path="model_best.pth.tar").to(device)
	...
	for i in range(len(process_stack)):
		...
		if data_args.slide_ext not in slide_name and 'tif' not in slide_name:
		...
		logits, A = infer_single_slide(model, features, label, label_dict, exp_args.n_classes)
		del features
		...
		process_stack.loc[i, 'Pred_{}'.format(label)] = logits
		# 	#process_stack.loc[i, 'p_{}'.format(c)] = Y_probs[c]

		os.makedirs('heatmaps/results/', exist_ok=True)
		if data_args.process_list is not None:
			process_stack.to_csv('heatmaps/results/{}brca.csv'.format(data_args.process_list.replace('brca.csv', '')), index=False)
		else:
			process_stack.to_csv('heatmaps/results/{}brca.csv'.format(exp_args.save_exp_code), index=False)
		...
		for sample in samples:
			if sample['sample']:
				tag = "label_{}_pred_{}".format(label, logits)
		...
		if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
			...
			heatmap.save(os.path.join(r_slide_save_dir, '{}_blockmap_{}.jpg'.format(slide_id,label)))
			del heatmap

		save_path = os.path.join(r_slide_save_dir, '{}_{}_roi_{}_{}.h5'.format(slide_id, patch_args.overlap, heatmap_args.use_roi,label))
		...
		if heatmap_args.calc_heatmap:
			compute_from_patches(wsi_object=wsi_object, clam_pred=label_dict[label], model=model, feature_extractor=feature_extractor, batch_size=exp_args.batch_size, **wsi_kwargs,
		...
		heatmap_save_name = '{}_{}_{}_roi_{}_blur_{}_rs_{}_bc_{}_a_{}_l_{}_bi_{}_{}.{}'.format(slide_id, label, float(patch_args.overlap), int(heatmap_args.use_roi),
